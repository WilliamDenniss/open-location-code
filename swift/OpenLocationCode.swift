...
/// CharacterSet represetnation of kCodeAlphabet.
...
let kSPCharset = CharacterSet.init(charactersIn: "+0")

/// Coordinates of a decoded Open Location Code.
/// The coordinates include the latitude and longitude of the lower left and
/// upper right corners and the center of the bounding box for the area the
/// code represents.
@objc public class OpenLocationCodeArea: NSObject {
...
@objc public class OpenLocationCode: NSObject {
  ...
  public static func isValid(code: String) -> Bool {
    ...
    let invalidChars = kCodeAlphabetCharset.union(kSPCharset).inverted
  ...
  ///   11.
  static func decodeGrid(_ code: String) -> OpenLocationCodeArea? {
  ...
  public static func recoverNearest(shortcode: String,
    ...
    return encode(latitude: codeArea.latitudeCenter,
  ...
  public static func shorten(code: String,
    ...
    for i in stride(from: kPairResolutions.count - 2, to: 0, by: -1) {
      // Check if we're close enough to shorten. The range must be less than 1/2
      // the resolution to shorten at all, and we want to allow some safety, so
      // use 0.3 instead of 0.5 as a multiplier.
      if coderange < (kPairResolutions[i] * 0.3) {
        // Trim it.
        let shortcode = code.substring(from: (i+1)*2)
        return shortcode
      }
    }
    return code
